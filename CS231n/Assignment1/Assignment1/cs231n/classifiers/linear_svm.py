...
def svm_loss_vectorized(W, X, y, reg):
    ...
    # 从评分矩阵中取出预计一致的评分，组成一个向量(500,)
    # np.arange(num_train)决定取scores的哪些行，y决定取scores的哪些列
    score_y = scores[np.arange(num_train), y]
    # 将 score_y 从(500,)变成（，500），再拓展成（500，10）
    ...
    #当j=yi的时候，dLi/dw_yi 应该等于Xi*一个系数(即mark）
    M[range(num_train),y]=-mark  
    # 数值矩阵加bool矩阵，相当于bool矩阵中为ture元素时，加1，false元素时加0
    M=M+sign
